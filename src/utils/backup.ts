...
// Backup file format: a plain JSON snapshot of saved spots + presets. The
// shape is versioned so a future format change can still read old files.
...
export async function exportBackup(): Promise<BackupCounts | null> {
  const [favorites, presets] = await Promise.all([loadFavorites(), loadPresets()]);
  if (favorites.length === 0 && presets.length === 0) return null;
  ...
  return { spots: favorites.length, presets: presets.length };
}
...
export async function importBackup(): Promise<BackupCounts | null> {
  ...
  if (spots > 0 || presets > 0) {
    importListeners.forEach((l) => l());
  }
  return { spots, presets };
}
